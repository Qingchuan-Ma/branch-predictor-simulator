...
void TAGE_Initial(TAGE* tage) //, uint32_t index_width)
{
    ...
    for (size_t i = 0; i < tageTableNum; i++)
    {
        ...
        for (size_t j = 0; j < num_row; j++)
        {
            ...
            tage->tage_tables[i].entry[j].valid = 0;
...
uint32_t TAGE_Compute_Tag(uint32_t unhashed_idx, uint64_t ghist, uint32_t table_index)
{
...
uint32_t TAGE_Compute_Index(uint32_t unhashed_idx, uint64_t ghist, uint32_t table_index)
{
    uint32_t hist_len = tageTableInfo[table_index][1];
...
Tage_Meta* TAGE_Predict(TAGE* tage, uint32_t unhashed_idx, uint64_t ghist) //, uint32_t* index_array, uint32_t* tag_array)
{
    int32_t provider = -1;
    uint32_t provided = 0;
    uint32_t altpred = 0;
    ...
    uint32_t alloc_entry = allocatable_slots[alloc_lfsr_index] ? lfsr_entry : first_entry;
...
{
    bool mispredict = result.actual_taken != result.predict_taken[TAGE];
    Tage_Meta* tage_meta = result.meta_info;

    if(tage_meta->provided) // 这里是更新provider
    {
        uint32_t index = tage_meta->index;
        uint32_t provider = tage_meta->provider;
        uint32_t provider_u = tage_meta->provider_u;
        uint32_t provider_new_u = TAGE_U_Update(tage_mate->alt_differ, mispredict, provider_u);
